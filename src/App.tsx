...
function App() {
  ...
  const cards = [cardMock1, cardMock2, cardMock3, cardMock4, cardMock5].map(
    (card) => new CardEntity(card)
  );

  return (
    <div className="flex flex-row items-center w-full h-full p-10">
      {cards.map((card) => (
        <div key={card.id} className="mr-10">
          <Card card={card} />
        </div>
      ))}
    </div>
